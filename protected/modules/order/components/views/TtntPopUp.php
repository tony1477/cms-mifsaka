<?php
	$sqldata="select t.*,a.gino,b.sono,c.fullname,d.companyname,e.currencyname,g.currencyrate,f.fullname as sales,g.invoiceno,h.docno 
				from ttntdetail t
				...
				where h.recordstatus = 3
					...
					b.companyid in (".getUserObjectValues('company').") 
				and g.invoiceno is not null 
				and g.payamount < g.amount and h.iscutar is null and h.iscbin is null
                ...
                and f.employeeid = '".(isset($_REQUEST['employeeid'])?$_REQUEST['employeeid']:'')."'
                and h.ttntid = '".(isset($_REQUEST['ttntid'])?$_REQUEST['ttntid']:'')."'
...
$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'invoiceid', 'invoiceno', 'fullname','sono','amount','payamount','sales'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		...
		'employeeid':$("input[name='<?php echo $this->RelationID2?>']").val(),
