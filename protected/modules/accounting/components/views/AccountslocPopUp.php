<?php
	$sqldata="select distinct a.accountid,a.accountcode,a.accountname,b.companyname
		from account a 
		inner join company b on b.companyid = a.companyid
		inner join plant c on c.companyid = b.companyid
		inner join sloc d on d.plantid = c.plantid
		where a.recordstatus = 1 
            and a.accounttypeid = 2
			and d.slocid = ".(isset($_REQUEST['slocid'])?$_REQUEST['slocid']:'null')." 
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'accountid', 'accountcode', 'accountname','companyname'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'accountcode':$("input[name='<?php echo $this->ColField?>_search_accountcode']").val(),
		'slocid':$("input[name='<?php echo $this->RelationID?>']").val(),
		'accountname':$("input[name='<?php echo $this->ColField?>_search_accountname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField?>_search_accountname"><?php echo $this->getCatalog('accountname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField?>_search_accountname" class="form-control">
					<span class="input-group-btn">
						<button name="<?php echo $this->ColField?>SearchButton" type="button" class="btn btn-primary" onclick="<?php echo $this->IDField?>searchdata()"><span class="glyphicon glyphicon-search"></span></button>
					</span>
				</div>
			</div>
			</div>
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField?>_search_accountcode"><?php echo $this->getCatalog('accountcode') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField?>_search_accountcode" class="form-control">
					</div>
					</div>
			</div>
			<?php	
