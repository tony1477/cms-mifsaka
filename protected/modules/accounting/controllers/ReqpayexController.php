...
class ReqpayexController extends AdminController
{
	protected $menuname = 'reqpayex';
	public $module = 'Accounting';
	protected $pageTitle = 'Permohonan Pembayaran Ekspedisi';
  ...
  ";
protected $sqldatareqpayinv = "select a0.reqpayinvid,a0.reqpayid,a0.invoiceapid,a0.ekspedisiid,a0.amount,a0.payamount,a0.taxid,a0.taxno,a0.taxdate,a0.currencyid,a0.currencyrate,a0.bankaccountno,a0.bankname,a0.bankowner,a0.itemnote,a1.ekspedisino as ekspedisino,a2.amount as amount,a3.taxcode as taxcode,a4.currencyname as currencyname 
    from reqpayinv a0 
    left join ekspedisi a1 on a1.ekspedisiid = a0.ekspedisiid
    left join ekspedisi a2 on a2.ekspedisiid = a0.amount
...
protected $sqlcountreqpayinv = "select count(1) 
  ...
  ";
	...
	protected function getSQL()
	{
		...
		if ((isset($_REQUEST['reqpayno'])) && (isset($_REQUEST['headernote'])) && (isset($_REQUEST['companyname'])))
		{				
			$where .=  " 
		...
		if ($error == false)
		{
			...
			try
			{
				if ($id !== '')
				{
			      ...
			      set ekspedisiid = :ekspedisiid,taxid = :taxid,taxno = :taxno,taxdate = :taxdate,currencyid = :currencyid,currencyrate = :currencyrate,bankaccountno = :bankaccountno,bankname = :bankname,bankowner = :bankowner,itemnote = :itemnote 
			      where reqpayinvid = :reqpayinvid';
				}
				else
				{
					$sql = 'insert into reqpayinv (ekspedisiid,taxid,taxno,taxdate,currencyid,currencyrate,bankaccountno,bankname,bankowner,itemnote) 
			      values (:ekspedisiid,:taxid,:taxno,:taxdate,:currencyid,:currencyrate,:bankaccountno,:bankname,:bankowner,:itemnote)';
				}
				$command = $connection->createCommand($sql);
				if ($id !== '')
				{
				...
				$command->bindvalue(':ekspedisiid',(($_POST['ekspedisiid']!=='')?$_POST['ekspedisiid']:null),PDO::PARAM_STR);
...
public function actionDelete()
    ...
    $this->pdf->companyid = $row['companyid'];
	  ...
	  $this->pdf->title=$this->getcatalog('reqpayex');
