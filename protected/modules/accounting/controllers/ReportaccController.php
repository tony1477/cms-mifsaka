...
class ReportaccController extends AdminController
{
	protected $menuname = 'reportacc';		
	public $module = 'accounting';
	protected $pageTitle = 'Laporan Akuntansi';
    ...
    */
	...
	public function RincianUmurPiutangGiro($companyid,$plantid,$sloc,$materialgroup,$customer,$supplier,$employee,$product,$account,$startaccode,$endacccode,$startdate,$enddate,$per)
	{
		...
		$sql = "select distinct b.addressbookid,b.fullname
						...
						where a.recordstatus = getwfmaxstatbywfname('appcheq') and a.companyid = ".$companyid." and a.iscustomer = 1
		...
		{	
			...
			$sql1 = "select *,
							...
							left join addressbook b on b.addressbookid=a.addressbookid
							left join bank c on c.bankid=a.bankid
							where a.recordstatus = getwfmaxstatbywfname('appcheq') and a.companyid = ".$companyid." and a.iscustomer = 1 and b.addressbookid = '".$row['addressbookid']."'
							and a.tglbayar <= '".date(Yii::app()->params['datetodb'], strtotime($enddate))."'
							and (a.tglcair is null or a.tglcair = '1970-01-01' or a.tglcair > '".date(Yii::app()->params['datetodb'], strtotime($enddate))."') 
							and (a.tgltolak is null or a.tgltolak = '1970-01-01' or a.tgltolak > '".date(Yii::app()->params['datetodb'], strtotime($enddate))."')
                            ".($_GET['plant']!='' ? ' and a.plantid = '.$_GET['plant'] : '')."
	...
	public function RincianGiroOpnameEkstern($companyid,$plantid,$sloc,$materialgroup,$customer,$supplier,$employee,$product,$account,$startaccode,$endacccode,$startdate,$enddate,$per)
	{
		...
		$sql = "select distinct b.addressbookid,b.fullname
						...
						left join addressbook b on b.addressbookid=a.addressbookid
						left join bank c on c.bankid=a.bankid
						where a.recordstatus = 2 and a.companyid = ".$companyid." and b.fullname like '%".$customer."%' and a.iscustomer = 1 and a.tglbayar <= '".date(Yii::app()->params['datetodb'], strtotime($enddate))."' and (a.tglcair is null or a.tglcair = '1970-01-01' or a.tglcair > '".date(Yii::app()->params['datetodb'], strtotime($enddate))."') and (a.tgltolak is null or a.tgltolak = '1970-01-01' or a.tgltolak > '".date(Yii::app()->params['datetodb'], strtotime($enddate))."')
                        ".($_GET['plant']!='' ? ' and a.plantid = '.$_GET['plant'] : '')."
		...
		$sql = "select distinct a.genjournalid,a.journalno,a.referenceno,a.journaldate,a.journalnote,a.recordstatus
			...
			foreach($dataReader as $row)
			{
				...
				$this->pdf->row(array(
					...
					$row['referenceno'],$row['journalnote'],Wfstatus::model()->findstatusname("apppayreq",$row['recordstatus'])
				));
		...
		parent::actionDownPDF();
			...
			foreach($dataReader as $row)
			{
				...
				$this->pdf->row(array(
					...
					$row['docno'],$row['headernote'],Wfstatus::model()->findstatusname("apppayreq",$row['recordstatus'])
				));
	...
	public function RekapPengeluaranKasBankPerDokumentBelumStatusMax($companyid,$plantid,$sloc,$materialgroup,$customer,$supplier,$employee,$product,$account,$startaccode,$endacccode,$startdate,$enddate,$per)
	{
		...
		foreach($dataReader as $row)
		{
			...
			$this->pdf->row(array(
				...
				date(Yii::app()->params['dateviewfromdb'], strtotime($row['docdate'])),
				$row['reqpayno'],$row['headernote'],Wfstatus::model()->findstatusname("apppayreq",$row['recordstatus'])
			));
	...
	{
		parent::actionDownPDF();
		$sql ="select distinct a.cbid,a.cashbankno,a.docdate,a.receiptno,a.headernote,a.recordstatus,a.statusname
					from cb a
					where a.docdate between '". date(Yii::app()->params['datetodb'], strtotime($startdate))."' 
					and '". date(Yii::app()->params['datetodb'], strtotime($enddate))."' 
			...
			foreach($dataReader as $row)
			{
				...
				$this->pdf->row(array(
					...
					date(Yii::app()->params['dateviewfromdb'], strtotime($row['docdate'])),
					$row['receiptno'],$row['headernote'],$row['statusname']
				));
	...
	public function LampiranNeraca1($companyid,$sloc,$materialgroup,$customer,$employee,$product,$account,$startaccode,$endacccode,$startdate,$enddate,$per)
	{
		...
		foreach($dataReader as $row)
		{
			...
			$saldoawal1=Yii::app()->db->createCommand($sql2)->queryScalar();
			...
			$debit1=Yii::app()->db->createCommand($sql3)->queryScalar();
			...
			$credit1=Yii::app()->db->createCommand($sql4)->queryScalar();
			...
			$saldoakhir1=Yii::app()->db->createCommand($sql5)->queryScalar();
			...
			foreach($dataReader1 as $row1)
			{
				...
				$saldoawal2=Yii::app()->db->createCommand($sql6)->queryScalar();
				...
				$debit2=Yii::app()->db->createCommand($sql7)->queryScalar();
				...
				$credit2=Yii::app()->db->createCommand($sql8)->queryScalar();
				...
				$saldoakhir2=Yii::app()->db->createCommand($sql9)->queryScalar();
...
*/        
	...
	/*//3
	public function NeracaUjiCobaXLS($companyid,$sloc,$materialgroup,$customer,$employee,$product,$account,$startaccode,$endacccode,$startdate,$enddate,$per)
	{
	...
	public function RekapJurnalUmumPerDokumenBelumStatusMaxXLS($companyid,$plantid,$sloc,$materialgroup,$customer,$supplier,$employee,$product,$account,$startaccode,$endacccode,$startdate,$enddate,$per)
	{
		...
		foreach($dataReader as $row)
		{
			...
			$this->phpExcel->setActiveSheetIndex(0)
					...
					->setCellValueByColumnAndRow(6,$line,Wfstatus::model()->findstatusname("apppayreq",$row['recordstatus']))	;
			$line++;
		}
