...
class CbController extends AdminController
{
	protected $menuname = 'cb';
	public $module = 'Accounting';
	protected $pageTitle = 'Cash Bank';
	public $wfname = 'appcb';
...
protected $sqlcountbank = "select count(1) 
  ...
  protected function getSQL()
  {
		...
		$where = "where a0.recordstatus in (".getUserRecordStatus('listcb').")
...
$dataProviderbank=new CSqlDataProvider($this->sqldatabank,array(
  ...
  {
		parent::actionCreate();
		$sql = "insert into cb (recordstatus) values (".$this->findstatusbyuser('inscb').")";
		...
		$id = Yii::app()->db->createCommand($sql)->queryScalar();echo CJSON::encode(array(
			...
			'cbid'=>$id,
			"docdate" =>date("Y-m-d"),
      "recordstatus" =>$this->findstatusbyuser("inscb")
		));
	}
  public function actionCreatecbacc()
  {
		parent::actionCreate();
  ...
  public function actionUpdate()
  {
		...
		{
			$id = $_POST['id'];
			$model = Yii::app()->db->createCommand($this->sqldata.' where a0.cbid = '.$id)->queryRow();
  ...
  public function actionUpdatecbacc()
  {
		parent::actionUpdate();
		if (isset($_POST['id']))
		{
			$id = $_POST['id'];
			$model = Yii::app()->db->createCommand($this->sqldatacbacc.' where cbaccid = '.$id)->queryRow();
			if ($model !== null)
			{
				echo CJSON::encode(array(
					'status'=>'success',
					'debitaccid'=>$model['debitaccid'],
          'creditaccid'=>$model['creditaccid'],
          'amount'=>$model['amount'],
          'currencyid'=>$model['currencyid'],
          'currencyrate'=>$model['currencyrate'],
          'chequeid'=>$model['chequeid'],
          'tglcair'=>$model['tglcair'],
          'tgltolak'=>$model['tgltolak'],
  ...
  public function actionUpdatecheque()
  {
		parent::actionUpdate();
		if (isset($_POST['id']))
		{
			$id = $_POST['id'];
			$model = Yii::app()->db->createCommand($this->sqldatacheque.' where chequeid = '.$id)->queryRow();
			if ($model !== null)
			{
				echo CJSON::encode(array(
					'status'=>'success',
					'companyid'=>$model['companyid'],
          'tglbayar'=>$model['tglbayar'],
          'chequeno'=>$model['chequeno'],
          'bankid'=>$model['bankid'],
          'amount'=>$model['amount'],
          'currencyid'=>$model['currencyid'],
          'currencyrate'=>$model['currencyrate'],
          'tglcheque'=>$model['tglcheque'],
  ...
  public function actionUpdatebank()
  {
      ...
      {
          $id = $_POST['id'];
          $model = Yii::app()->db->createCommand($this->sqldatabank.' where bankid = '.$id)->queryRow();
          if ($model !== null)
          {
              echo CJSON::encode(array(
                  'status'=>'success',
  ...
  public function actionSave()
  {
    ...
    array('companyid','string','emptycompanyid'),
      ...
      if ($error == false)
      {
          ...
          try
          {
              $sql = 'call ApproveCB (:docdate
,:companyid
,:receiptno
,:isin
,:headernote,:vcreatedby)';
              $command = $connection->createCommand($sql);
              $command->bindvalue(':cbid',$_POST['cbid'],PDO::PARAM_STR);
      ...
      $command->bindvalue(':receiptno',(($_POST['receiptno']!=='')?$_POST['receiptno']:null),PDO::PARAM_STR);
      $command->bindvalue(':isin',(($_POST['isin']!=='')?$_POST['isin']:null),PDO::PARAM_STR);
  ...
  public function actionSavecbacc()
  {
      parent::actionSave();
      $error = $this->ValidateData(array(
          array('debitaccid','string','emptydebitaccid'),
    array('creditaccid','string','emptycreditaccid'),
    ...
    array('currencyrate','string','emptycurrencyrate'),
      ...
      if ($error == false)
      {
          ...
          try
          {
              ...
              {
                  $sql = 'update cbacc 
                set debitaccid = :debitaccid,creditaccid = :creditaccid,amount = :amount,currencyid = :currencyid,currencyrate = :currencyrate,chequeid = :chequeid,tglcair = :tglcair,tgltolak = :tgltolak,description = :description 
                where cbaccid = :cbaccid';
              }
              else
              {
                  $sql = 'insert into cbacc (debitaccid,creditaccid,amount,currencyid,currencyrate,chequeid,tglcair,tgltolak,description) 
                values (:debitaccid,:creditaccid,:amount,:currencyid,:currencyrate,:chequeid,:tglcair,:tgltolak,:description)';
              }
              $command = $connection->createCommand($sql);
              if ($id !== '')
              {
                  $command->bindvalue(':cbaccid',$_POST['cbaccid'],PDO::PARAM_STR);
              }
              $command->bindvalue(':debitaccid',(($_POST['debitaccid']!=='')?$_POST['debitaccid']:null),PDO::PARAM_STR);
      $command->bindvalue(':creditaccid',(($_POST['creditaccid']!=='')?$_POST['creditaccid']:null),PDO::PARAM_STR);
      ...
      $command->bindvalue(':tgltolak',(($_POST['tgltolak']!=='')?$_POST['tgltolak']:null),PDO::PARAM_STR);
  ...
  public function actionSavecheque()
  {
      ...
      $error = $this->ValidateData(array(
          array('companyid','string','emptycompanyid'),
    array('tglbayar','string','emptytglbayar'),
    array('chequeno','string','emptychequeno'),
    array('bankid','string','emptybankid'),
    array('amount','string','emptyamount'),
    array('currencyid','string','emptycurrencyid'),
    array('currencyrate','string','emptycurrencyrate'),
    array('tglcheque','string','emptytglcheque'),
    array('tgltempo','string','emptytgltempo'),
    array('addressbookid','string','emptyaddressbookid'),
      ...
      if ($error == false)
      {
          ...
          try
          {
              ...
              {
                  $sql = 'update cheque 
                set companyid = :companyid,tglbayar = :tglbayar,chequeno = :chequeno,bankid = :bankid,amount = :amount,currencyid = :currencyid,currencyrate = :currencyrate,tglcheque = :tglcheque,tgltempo = :tgltempo,tglcair = :tglcair,tgltolak = :tgltolak,addressbookid = :addressbookid,iscustomer = :iscustomer 
                where chequeid = :chequeid';
              }
              else
              {
                  $sql = 'insert into cheque (companyid,tglbayar,chequeno,bankid,amount,currencyid,currencyrate,tglcheque,tgltempo,tglcair,tgltolak,addressbookid,iscustomer) 
                values (:companyid,:tglbayar,:chequeno,:bankid,:amount,:currencyid,:currencyrate,:tglcheque,:tgltempo,:tglcair,:tgltolak,:addressbookid,:iscustomer)';
              }
              $command = $connection->createCommand($sql);
              if ($id !== '')
              {
                  $command->bindvalue(':chequeid',$_POST['chequeid'],PDO::PARAM_STR);
              }
              $command->bindvalue(':companyid',(($_POST['companyid']!=='')?$_POST['companyid']:null),PDO::PARAM_STR);
      $command->bindvalue(':tglbayar',(($_POST['tglbayar']!=='')?$_POST['tglbayar']:null),PDO::PARAM_STR);
      $command->bindvalue(':chequeno',(($_POST['chequeno']!=='')?$_POST['chequeno']:null),PDO::PARAM_STR);
      $command->bindvalue(':bankid',(($_POST['bankid']!=='')?$_POST['bankid']:null),PDO::PARAM_STR);
      $command->bindvalue(':amount',(($_POST['amount']!=='')?$_POST['amount']:null),PDO::PARAM_STR);
      $command->bindvalue(':currencyid',(($_POST['currencyid']!=='')?$_POST['currencyid']:null),PDO::PARAM_STR);
      $command->bindvalue(':currencyrate',(($_POST['currencyrate']!=='')?$_POST['currencyrate']:null),PDO::PARAM_STR);
      $command->bindvalue(':tglcheque',(($_POST['tglcheque']!=='')?$_POST['tglcheque']:null),PDO::PARAM_STR);
      $command->bindvalue(':tgltempo',(($_POST['tgltempo']!=='')?$_POST['tgltempo']:null),PDO::PARAM_STR);
      $command->bindvalue(':tglcair',(($_POST['tglcair']!=='')?$_POST['tglcair']:null),PDO::PARAM_STR);
      $command->bindvalue(':tgltolak',(($_POST['tgltolak']!=='')?$_POST['tgltolak']:null),PDO::PARAM_STR);
  ...
  public function actionSavebank()
  {
      ...
      if ($error == false)
      {
          ...
          try
          {
              ...
              {
                  $sql = 'insert into bank (bankname) 
                values (:bankname)';
              }
              $command = $connection->createCommand($sql);
              if ($id !== '')
              {
                  $command->bindvalue(':bankid',$_POST['bankid'],PDO::PARAM_STR);
              }
  ...
  public function actionPurge()
  {
      ...
      try
      {
          if (isset($_POST['id']))
          {
              ...
              {
              $sql = "delete from cb where cbid = ".$id[$i];
              Yii::app()->db->createCommand($sql)->execute();
              }
              $transaction->commit();
              $this->getMessage('success','alreadysaved');
  ...
  public function actionPurgecbacc()
  {
      parent::actionPurge();
      $connection=Yii::app()->db;
      $transaction=$connection->beginTransaction();
      try
      {
          if (isset($_POST['id']))
          {
              ...
              for ($i = 0; $i < count($_POST['id']);$i++)
              {
              $sql = "delete from cbacc where cbaccid = ".$id[$i];
              Yii::app()->db->createCommand($sql)->execute();
              }
              $transaction->commit();
              $this->getMessage('success','alreadysaved');
  ...
  public function actionPurgecheque()
  {
      ...
      try
      {
          if (isset($_POST['id']))
          {
              ...
              for ($i = 0; $i < count($_POST['id']);$i++)
              {
              $sql = "delete from cheque where chequeid = ".$id[$i];
              Yii::app()->db->createCommand($sql)->execute();
              }
              $transaction->commit();
              $this->getMessage('success','alreadysaved');
  ...
  public function actionPurgebank()
  {
      ...
      try
      {
          if (isset($_POST['id']))
          {
              ...
              for ($i = 0; $i < count($_POST['id']);$i++)
              {
              $sql = "delete from bank where bankid = ".$id[$i];
              Yii::app()->db->createCommand($sql)->execute();
              }
              $transaction->commit();
              $this->getMessage('success','alreadysaved');
  ...
  public function actionDownPDF()
  {
      ...
      foreach($dataReader as $row)
      {
              ...
              $sql1 = "select b.accountname as accdebitname,c.accountname as acccreditname,case when a.employeeid <> 0 then concat(a.description,' - ',f.fullname) else a.description end as description,a.amount,
                  d.currencyname,e.chequeno,f.fullname,
                  ...
                  left join employeeid f on f.employeeid=a.employeeid
