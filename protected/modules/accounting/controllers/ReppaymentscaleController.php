...
class ReppaymentscaleController extends AdminController
{
	protected $menuname = 'reppaymentscale';		
	public $module = 'order';
	protected $pageTitle = 'Daftar Skala Tagihan';
	public $wfname = 'listps';
	...
	protected $sqldatapaymentscaledet = "select t.paymentscaledetid,t.paymentscaleid,t.min,t.max,t.gt30,t.gt15,t.gt7,t.gt0,t.x0,t.lt0,t.lt14, concat('>= ',t.min,' & < ',t.max) as `range`
         from paymentscaledet t ";
	...
	protected function getSQL()
	{
		...
		if ((isset($_REQUEST['paymentscaleid'])) && $_REQUEST['paymentscaleid']!=='' && $_REQUEST['paymentscaleid']!=0)
		{				   
            $where .=" and (coalesce(paymentscaleid,'') like {$_REQUEST['paymentscaleid']})";
        }
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CSqlDataProvider($this->sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'paymentscaleid','companyid','docno','docdate','perioddate', 'paramspv','minscale','recordstatus','spvscale'
        ),
				'defaultOrder' => array( 
		...
		if (isset($_REQUEST['paymentscaledetid']))
		{
			$this->sqlcountpaymentscaledet .= ' where paymentscaledetid = '.$_REQUEST['paymentscaledetid'];
			$this->sqldatapaymentscaledet .= ' where paymentscaledetid = '.$_REQUEST['paymentscaledetid'];
		}
		if (isset($_REQUEST['paymentscaleid']))
		{
			$this->sqlcountpaymentscaledet .= ' where paymentscaleid = '.$_REQUEST['paymentscaleid'];
			$this->sqldatapaymentscaledet .= ' where paymentscaleid = '.$_REQUEST['paymentscaleid'];
			$this->sqlcountpaymentscalecat .= ' where paymentscaleid = '.$_REQUEST['paymentscaleid'];
			$this->sqldatapaymentscalecat .= ' where paymentscaleid = '.$_REQUEST['paymentscaleid'];
		}
		...
		$dataProviderpaymentscaledet=new CSqlDataProvider($this->sqldatapaymentscaledet,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'paymentscaledetid', 'min', 'max', 'gt30', 'gt15', 'gt7', 'gt0','x0','lt0','lt14'
        ),
				'defaultOrder' => array( 
					'paymentscaledetid' => CSort::SORT_ASC
				),
			),
		));
