...
+ '&addressbook='+$("input[name='fullname']").val()
+ '&startdate='+$("input[name='startdate']").val()
+ '&enddate='+$("input[name='enddate']").val()
+'&per=10';
	window.open('<?php echo Yii::app()->createUrl('accounting/reprealpay/downpdf')?>?'+array);
	}
};
...
+ '&enddate='+$("input[name='enddate']").val()
+'&per=10';
	window.open('<?php echo Yii::app()->createUrl('accounting/reprealpay/downxls')?>?'+array);
	}
};
...
<h3><?php echo $this->getCatalog('reprealpay') ?></h3>
			<?php $this->widget('DataPopUp',
					...
					?>
						...
						array('id'=>'Widget','IDField'=>'addressbookid','ColField'=>'fullname',
							...
							'classtypebox'=>'col-md-8',
							'PopUpName'=>'common.components.views.SupplierPopUp','PopGrid'=>'addressbookidgrid')); 
					?>
