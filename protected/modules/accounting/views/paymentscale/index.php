...
{
    jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('accounting/paymentscale/checkdetail')?>','data':{'id':$("input[name='paymentscale_0_paymentscaleid']").val()},
		...
		'success':function(data) {
			if (data.status == "success")
			{
                ...
                $.fn.yiiGridView.update("paymentscalecatList");
			}
			else
			{
...
function updatedata($id)
{
	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('accounting/paymentscale/update')?>','data':{'id':$id},
      ...
      $("input[name='paymentscale_0_spvscale']").val(data.spvscale);
      $("input[name='paymentscale_0_companyname']").val(data.companyname);
				$.fn.yiiGridView.update('paymentscaledetList',{data:{'paymentscaleid':data.paymentscaleid}});
                $.fn.yiiGridView.update('paymentscalecatList',{data:{'paymentscaleid':data.paymentscaleid}});
...
function updatedatapaymentscalecat($id)
{
	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('accounting/paymentscale/updatepaymentscalecat')?>','data':{'id':$id},
		'type':'post','dataType':'json',
		'success':function(data) {
			if (data.status == "success")
			{
                $("input[name='paymentscalecat_2_paymentscalecatid']").val(data.paymentscalecatid);
                $("input[name='paymentscalecat_2_custcategoryid']").val(data.custcategoryid);
                $("input[name='paymentscalecat_2_custcategoryname']").val(data.custcategoryname);
                $("input[name='paymentscalecat_2_paramvalue']").val(data.paramvalue);
                $('#InputDialogpaymentscalecat').modal();
			}
			else
			{
				toastr.error(data.div);
...
function savedata()
{
	...
	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('accounting/paymentscale/save')?>',
      ...
      'spvscale':$("input[name='paymentscale_0_spvscale']").val(),
		},
		'type':'post','dataType':'json',
		'success':function(data) {
			if (data.status == "success")
...
function savedatapaymentscalecat()
{

	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('accounting/paymentscale/savepaymentscalecat')?>',
		'data':{
            'paymentscalecatid':$("input[name='paymentscalecat_2_paymentscalecatid']").val(),
            'paymentscaleid':$("input[name='paymentscale_0_paymentscaleid']").val(),
            'custcategoryid':$("input[name='paymentscalecat_2_custcategoryid']").val(),
            'paramvalue':$("input[name='paymentscalecat_2_paramvalue']").val(),
        },
		'type':'post','dataType':'json',
		'success':function(data) {
			if (data.status == "success")
			{
				$('#InputDialogpaymentscalecat').modal('hide');
				toastr.info(data.div);
				$.fn.yiiGridView.update("paymentscalecatList");
			}
			else
...
<div id="InputDialog" class="modal fade" role="dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
						<div class="col-md-4">
							<label for="paymentscale_0_spvscale"><?php echo $this->getCatalog('spvscale')?></label>
						</div>
						<div class="col-md-8">
							<input type="number" class="form-control" name="paymentscale_0_spvscale">
						</div>
					</div>
							
				<ul class="nav nav-tabs">
				<li><a data-toggle="tab" href="#paymentscaledet" onclick="checkdetail()"><?php echo $this->getCatalog("generate")?></a></li>
...
<button name="PurgeButtonpaymentscaledet" type="button" class="btn btn-danger" onclick="purgedatapaymentscaledet()"><?php echo $this->getCatalog('purge')?></button> -->
    ...
    <?php
        $this->widget('zii.widgets.grid.CGridView', array(
        'dataProvider'=>$dataProviderpaymentscalecat,
            'id'=>'paymentscalecatList',
            'selectableRows'=>2,
        'ajaxUpdate'=>true,
            'filter'=>null,
            'enableSorting'=>true,
                ...
                (
                    ...
                    (
                        ...
                        (
                                ...
                                'url'=>'"#"',
                                'click'=>"function() { 
                                    updatedatapaymentscalecat($(this).parent().parent().children(':nth-child(3)').text());
                                }",
                        ),
                        'purge' => array
                        (
                                ...
                                'url'=>'"#"',
                                'click'=>"function() { 
                                    purgedatapaymentscalecat($(this).parent().parent().children(':nth-child(3)').text());
                                }",
                        ),
                    ),
                ),
                array(
                    'header'=>$this->getCatalog('paymentscalecatid'),
                    'name'=>'paymentscalecatid',
                    'value'=>'$data["paymentscalecatid"]'
                ),
                array(
    ...
    ?>
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><?php echo GetCatalog('paymentscaledet')?></h3>
							<div class="box-tools pull-right">
								<button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
							</div>
						</div><!-- /.box-header -->
						<div class="box-body">
                        <button name="CreateButton" type="button" class="btn btn-primary" onclick="newdatapaymentscaledet()"><?php echo $this->getCatalog('new')?></button>
                        <button name="CreateButton" type="button" class="btn btn-danger" onclick="purgedatapaymentscaledet($.fn.yiiGridView.getSelection('paymentscaledetList'))"><?php echo $this->getCatalog('purge')?></button>
    <?php
	$this->widget('zii.widgets.grid.CGridView', array(
...
<div id="InputDialogpaymentscaledet" class="modal fade" role="dialog">
    ...
    <div id="InputDialogpaymentscalecat" class="modal fade" role="dialog">
	<div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title"><?php echo $this->getCatalog('paymentscalecat') ?></h4>
      </div>
			<div class="modal-body">
			<input type="hidden" class="form-control" name="paymentscalecat_2_paymentscalecatid">
        <?php $this->widget('DataPopUp',
						array('id'=>'Widget','IDField'=>'paymentscalecat_2_custcategoryid','ColField'=>'paymentscalecat_2_custcategoryname',
							'IDDialog'=>'paymentscalecat_2_custcategoryid_dialog','titledialog'=>$this->getCatalog('custcategory'),'classtype'=>'col-md-4',
							'classtypebox'=>'col-md-8',
							'PopUpName'=>'common.components.views.CustcategoryPopUp','PopGrid'=>'paymentscalecat_2_custcategoryidgrid')); 
					?>
							
        <div class="row">
						<div class="col-md-4">
							<label for="paymentscalecat_2_paramvalue"><?php echo $this->getCatalog('paramvalue')?></label>
						</div>
						<div class="col-md-8">
							<input type="number" class="form-control" name="paymentscalecat_2_paramvalue">
						</div>
					</div>
							
			</div>
			      <div class="modal-footer">
				<button type="submit" class="btn btn-success" onclick="savedatapaymentscalecat()"><?php echo $this->getCatalog('save') ?></button>
        <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo $this->getCatalog('close') ?></button>
      </div>
			</div>
			</div>
