<?php
	$sqldata="select distinct a.slocid,a.plantid,a.sloccode,a.description
		from sloc a 
		join productplandetail b on b.fromslocid = a.slocid 
		join product c on c.productid = b.detailproductid 
		where a.recordstatus = 1 and c.isstock = 1 and a.sloccode like '%".(isset($_REQUEST['sloccode'])?$_REQUEST['sloccode']:'')."%' 
		and a.description like '%".(isset($_REQUEST['description'])?$_REQUEST['description']:'')."%' 
		and b.productplanid = ".(isset($_REQUEST['productplanid'])?$_REQUEST['productplanid']:'null')."
		and slocid in (".getUserObjectValues('sloc').")";
	...
	$product=new CSqlDataProvider($sqldata,array(
		...
		?>
<script>
function <?php echo $this->IDField?>getdata($id)
{
	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('common/sloc/update')?>',
		'data':{'id':$id},
		'type':'post','dataType':'json',
		'success':function(data) {
			if (data.status == "success")
			{
				$("input[name='<?php echo $this->IDField ?>']").val(data.slocid);
				$("input[name='<?php echo $this->ColField ?>']").val(data.sloccode+'-'+data.description);
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		...
		'productplanid':$("input[name='<?php echo $this->RelationID?>']").val(),
...
$(document).ready(function(){
	...
	$(":input[name*='<?php echo $this->ColField; ?>_search_sloccode']").keyup(function(e){
	if(e.keyCode == 13)
	{
		<?php echo $this->IDField?>searchdata()
	}});
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->IDField?>_search_sloccode"><?php echo $this->getCatalog('sloccode') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
