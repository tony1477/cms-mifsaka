<?php
	$sqldata="select t.productoutputid,t.productoutputno
		from productoutput t
		where t.productoutputno like '%".(isset($_REQUEST['productoutputno'])?$_REQUEST['productoutputno']:'')."%'
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'productoutputid', 'productoutputno'
        ),
				'defaultOrder' => array( 
...
$(document).ready(function(){
	...
	$(":input[name*='<?php echo $this->ColField; ?>_search_productoutputno']").keyup(function(e){
	if(e.keyCode == 13)
	{
		<?php echo $this->IDField?>searchdata()
	}});
