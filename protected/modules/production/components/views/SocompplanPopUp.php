<?php
	$sqldata="select a.soheaderid, a.sono, c.fullname, b.companyname, a.pocustno
	  ...
	  join company b on b.companyid = a.companyid 
		...
		where a.pocustno is not null and b.companyname like '%".(isset($_REQUEST['companyname'])?$_REQUEST['companyname']:'')."%' 
...
{
	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('order/soheader/getdata')?>',
		'data':{'id':$id},
		'type':'post','dataType':'json',
		'success':function(data) {
			if (data.status == "success")
			{
				$("input[name='<?php echo $this->IDField ?>']").val(data.soheaderid);
				$("input[name='<?php echo $this->ColField ?>']").val(data.sono);
