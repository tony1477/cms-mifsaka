...
class ReporthrController extends AdminController
{
	protected $menuname = 'reporthr';		
	public $module = 'hr';
	protected $pageTitle = 'Laporan HRD';
    ...
    */
	...
	public function no_result(){
			...
			$this->pdf->text(10, $this->pdf->gety() + 5, 'Tidak ditemukan Laporan data dengan');
			$this->pdf->sety($this->pdf->gety()+15);
			$connection = Yii::app()->db;
			if($employeeid!=''){
					 $getemployee = "SELECT GetEmpNameByEmpID($employeeid)";
					 $employeename = $connection->createCommand($getemployee)->queryScalar();
					 $this->pdf->text(10, $this->pdf->gety(), 'Nama Karyawan : '.$employeename);
					 $this->pdf->sety($this->pdf->gety()+15);
			}
			if($empplanid!=''){
					$getempname = "SELECT empplanname FROM empplan WHERE empplanid='".$empplanid."'";
					$empplanname = $connection->createCommand($getempname)->queryScalar();
					$this->pdf->text(10, $this->pdf->gety(), 'Nama Kegiatan/ Rencana : '.$empplanname);
					 $this->pdf->sety($this->pdf->gety()+15);
			}
			$this->pdf->text(10, $this->pdf->gety(), 'Tanggal: '.$startdate.' - '.$enddate);
	...
	public function RekapDataKaryawan($companyid,$employeeid,$religionid,$employeetypeid,$empplanid,$startdate,$enddate)
	{
			...
			foreach($dataReader as $row)
			{
					...
					*/
					...
					$this->pdf->setwidths(array(10,30,30,25,35,30,30,30));
					$this->pdf->colheader = array('No','Nama Perusahaan','Alamat','Posisi Awal','Posisi Terakhir','Awal Masuk','Tanggal Keluar ','reasonleave');
					$this->pdf->RowHeader();
					$this->pdf->coldetailalign = array('L','L','L','L','L','L','L','L');
					$i6=0;
					$this->pdf->setFont('Arial','',9);
	...
	public function RekapDataKaryawanXLS($companyid,$employeeid,$religionid,$employeetypeid,$startdate,$enddate,$per)
	{
		...
		->setCellValueByColumnAndRow(5,1,$this->GetCompanyCode($companyid));
			...
			->setCellValueByColumnAndRow(0,$line,'Pengalaman Kerja Karyawan');
					...
					$this->phpExcel->setActiveSheetIndex(0)
									->setCellValueByColumnAndRow(0,$line,'No')
									->setCellValueByColumnAndRow(1,$line,'Nama Perusahaan')
									->setCellValueByColumnAndRow(2,$line,'Alamat')
									->setCellValueByColumnAndRow(3,$line,'Posisi Awal')
									->setCellValueByColumnAndRow(4,$line,'Posisi Terakhir')
									->setCellValueByColumnAndRow(5,$line,'Awal Masuk')
									->setCellValueByColumnAndRow(6,$line,'Tanggal Keluar')
									->setCellValueByColumnAndRow(7,$line,'Alasan Keluar');
					$line++;
					$i6=1;
					foreach($dataReader6 as $row6)
					{
                        $this->phpExcel->setActiveSheetIndex(0)
									...
									->setCellValueByColumnAndRow(1,$line,$row6['employer'])
									->setCellValueByColumnAndRow(2,$line,$row6['addressname'])
									->setCellValueByColumnAndRow(3,$line,$row6['firstposition'])
									->setCellValueByColumnAndRow(4,$line,$row6['lastposition'])
									->setCellValueByColumnAndRow(5,$line,$row6['startdate'])
									->setCellValueByColumnAndRow(6,$line,$row6['enddate'])
									->setCellValueByColumnAndRow(7,$line,$row6['reasonleave']);
							$line++;
							$i6++;
					}
