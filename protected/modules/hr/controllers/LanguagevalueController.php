...
class LanguagevalueController extends AdminController
{
	protected $menuname = 'languagevalue';
	public $module = 'Hr';
	protected $pageTitle = 'Nilai Kemampuan Bahasa';
	public $wfname = '';
	protected $sqldata = "select a0.languageid,a0.languagename,a0.recordstatus,getwfstatusbywfname('',a0.recordstatus) as statusname  
    from language a0 
  ";
...
public function actionApprove()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				$sql = 'call Approvelanguagevalue(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
...
public function actionDelete()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				$sql = 'call Deletelanguagevalue(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
					$command->bindvalue(':vid',$ids,PDO::PARAM_STR);
	...
	public function actionDownPDF()
	{
		...
		//masukkan judul
		$this->pdf->title=$this->getCatalog('languagevalue');
		$this->pdf->AddPage('P');
