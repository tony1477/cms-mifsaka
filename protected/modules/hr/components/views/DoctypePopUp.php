<?php
	$sqldata="select doctypeid,doctypename 
		from doctype 
		where recordstatus = 1 and doctypename like '%".(isset($_REQUEST['doctypename'])?$_REQUEST['doctypename']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$doctype=new CSqlDataProvider($sqldata,array(
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'doctypename':$("input[name='<?php echo $this->ColField; ?>_search_doctypename']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_doctypename"><?php echo $this->getCatalog('doctypename') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_doctypename" class="form-control">
					<span class="input-group-btn">
