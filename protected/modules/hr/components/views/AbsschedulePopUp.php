<?php
	$sqldata="SELECT absscheduleid, absschedulename, absin, absout, a.absstatusid, b.longstat
            FROM absschedule a
            left join absstatus b on b.absstatusid = a.absstatusid
            WHERE absschedulename LIKE '%". (isset($_REQUEST['absschedulename'])?$_REQUEST['absschedulename']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$absschedule=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'absscheduleid', 'absschedulename','absin','absout','longstat'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'absschedulename':$("input[name='<?php echo $this->ColField; ?>_search_absschedulename']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_absschedulename"><?php echo $this->getCatalog('absschedulename') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_absschedulename" class="form-control">
					<span class="input-group-btn">
