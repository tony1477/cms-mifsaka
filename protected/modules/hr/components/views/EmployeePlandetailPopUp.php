<?php
	$sqldata="SELECT DISTINCT b.*, c.fullname 
             FROM empplandetail b
             LEFT JOIN employee c ON c.employeeid = b.employeeid
             WHERE empplanid = ".(isset($_REQUEST['empplanid'])?$_REQUEST['empplanid']:'null')." 
             AND b.description LIKE '%".(isset($_REQUEST['description']) ? $_REQUEST['description'] : '')."%'
	...
	$employeeplan=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
            'empplandetailid','description','fullname','startdate','enddate'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
        ...
        'empplanid':$("input[name='<?php echo $this->RelationID?>']").val(),
      ...
      <div class="modal-body">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
                    ...
                    array(
						'header'=>$this->getCatalog('stardate'),
						'name'=>'startdate', 
