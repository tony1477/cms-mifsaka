<?php
	$sqldata="select languagevalueid,languagevaluename 
		from languagevalue 
		where recordstatus = 1 and languagevaluename like '%".(isset($_REQUEST['languagevaluename'])?$_REQUEST['languagevaluename']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$languagevalue=new CSqlDataProvider($sqldata,array(
			'totalItemCount'=>$count,
			'keyField'=>'languagevalueid',
			'pagination'=>array(
			...
			'sort'=>array(
        'attributes'=>array(
             'languagevalueid', 'languagevaluename'
        ),
				'defaultOrder' => array( 
					'languagevalueid' => CSort::SORT_DESC
				),
			),
		));
		?>
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'languagevaluename':$("input[name='<?php echo $this->ColField; ?>_search_languagevaluename']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_languagevaluename"><?php echo $this->getCatalog('languagevaluename') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_languagevaluename" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				'id'=>$this->PopGrid,
				'selectableRows'=>1,
				'dataProvider'=>$languagevalue,
				'selectionChanged'=>'function(id){'.$this->PopGrid.'onSelectionChange()}',
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('languagevalueid'),
						'name'=>'languagevalueid', 
						'value'=>'$data["languagevalueid"]',
						'htmlOptions'=>array('width'=>'1%')
					),
					array(
						'header'=>$this->getCatalog('languagevaluename'),
						'name'=>'languagevaluename', 
						'value'=>'$data["languagevaluename"]',
					),
				),
			));?>
			</div>
