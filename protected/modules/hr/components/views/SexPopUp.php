<?php
	$sqldata="select sexid,sexname 
		from sex 
		where recordstatus = 1 and sexname like '%".(isset($_REQUEST['sexname'])?$_REQUEST['sexname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$sex=new CSqlDataProvider($sqldata,array(
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'sexname':$("input[name='<?php echo $this->ColField; ?>_search_sexname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_sexname"><?php echo $this->getCatalog('sexname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_sexname" class="form-control">
					<span class="input-group-btn">
