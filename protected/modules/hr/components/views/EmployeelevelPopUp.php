<?php
	$sqldata="SELECT e.fullname, e.employeeid, u.groupaccessid FROM groupmenuauth gm
INNER JOIN menuauth m ON m.menuauthid = gm.menuauthid
INNER JOIN groupaccess g ON g.groupaccessid = gm.groupaccessid
INNER JOIN usergroup u ON u.groupaccessid = g.groupaccessid
INNER JOIN useraccess us ON us.useraccessid = u.useraccessid AND us.useraccessid = u.useraccessid 
INNER JOIN employee e ON e.employeeid = gm.menuvalueid
WHERE upper(us.username) = upper('".Yii::app()->user->name."') AND upper(m.menuobject) = upper('employee')";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$employeetype=new CSqlDataProvider($sqldata,array(
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'employeename':$("input[name='<?php echo $this->ColField; ?>_search_employeename']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_employeeplanname"><?php echo $this->getCatalog('employeeplanname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_employeeplanname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					...
					array(
						'header'=>$this->getCatalog('employeeplanname'),
						'name'=>'employeeplanname', 
						'value'=>'$data["fullname"]',
					),
				),
			));?>
