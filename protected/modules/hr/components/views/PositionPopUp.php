<?php
	$sqldata="select positionid,positionname 
		from position 
		where recordstatus = 1 and positionname like '%".(isset($_REQUEST['positionname'])?$_REQUEST['positionname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$position=new CSqlDataProvider($sqldata,array(
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'positionname':$("input[name='<?php echo $this->ColField; ?>_search_positionname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_positionname"><?php echo $this->getCatalog('positionname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_positionname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('positionid'),
						'name'=>'positionid', 
