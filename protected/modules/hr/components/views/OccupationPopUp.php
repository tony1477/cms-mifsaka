<?php
	$sqldata="select occupationid,occupationname 
		from occupation 
		where recordstatus = 1 and occupationname like '%".(isset($_REQUEST['occupationname'])?$_REQUEST['occupationname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$occupation=new CSqlDataProvider($sqldata,array(
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'occupationname':$("input[name='<?php echo $this->ColField; ?>_search_occupationname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_occupationname"><?php echo $this->getCatalog('occupationname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_occupationname" class="form-control">
					<span class="input-group-btn">
