<?php
	$sqldata="select distinct a.employeeid,a.fullname,a.oldnik, c.structurename, a.teamno, d.gradename 
		from employee a 
		inner join company b on b.companyid = a.companyid 
		left join orgstructure c on c.orgstructureid = a.orgstructureid 
		left join grade d on d.gradeid = a.gradeid 		
		where c.recordstatus = 1 
			and a.fullname like '%".(isset($_REQUEST['fullname'])?$_REQUEST['fullname']:'')."%'
			and b.companyid in (select gm.menuvalueid from groupaccess c
...
and c.orgstructureid in (select gm.menuvalueid from groupaccess c
inner join usergroup d on d.groupaccessid = c.groupaccessid
inner join useraccess e on e.useraccessid = d.useraccessid
inner join groupmenuauth gm on gm.groupaccessid = c.groupaccessid
inner join menuauth ma on ma.menuauthid = gm.menuauthid
where upper(e.username)=upper('".Yii::app()->user->name."') and upper(ma.menuobject) = upper('orgstructure'))
			";
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'employeeid', 'fullname', 'oldnik', 'structurename','teamno','gradename'
        ),
				'defaultOrder' => array( 
