<?php
	$sqldata="select a.orgstructureid, substring_index(substring_index(structurename, ',', 1), ',', - 1) as structurename, a.companyid, b.companyname, substring_index(substring_index(structurename, ',', 2), ',', - 1) as department,  substring_index(substring_index(structurename, ',', 3), ',', - 1) as divisi
		from orgstructure a 
		left join company b on b.companyid = a.companyid 
		where a.recordstatus = 1
        AND b.companyid IN (SELECT DISTINCT gm.menuvalueid FROM groupmenuauth gm
                ...
                LEFT JOIN company e ON e.companyid = gm.menuvalueid
                WHERE us.username='".Yii::app()->user->id."' AND m.menuobject='company') 
                AND structurename like '%".(isset($_REQUEST['structurename'])?$_REQUEST['structurename']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$orgstructure=new CSqlDataProvider($sqldata,array(
