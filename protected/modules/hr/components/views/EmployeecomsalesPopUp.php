<?php
	$sqldata="select a.employeeid,substring_index(substring_index(structurename, ',', 1), ',', - 1) as structurename,b.orgstructureid, c.fullname
        from employeeorgstruc a
        join orgstructure b on b.orgstructureid = a.orgstructureid
        join employee c on c.employeeid = a.employeeid
        where a.recordstatus = 1 and b.orgstructureid in (select x.orgstructureid
			from orgstructure x
			join employeeorgstruc y on y.orgstructureid = x.orgstructureid
			join employee z on z.employeeid = y.employeeid
			where z.employeeid in (select gm.menuvalueid from groupaccess c
				join usergroup d on d.groupaccessid = c.groupaccessid
				join useraccess e on e.useraccessid = d.useraccessid
				join groupmenuauth gm on gm.groupaccessid = c.groupaccessid
				join menuauth ma on ma.menuauthid = gm.menuauthid
				where e.username = '".Yii::app()->user->id."' and ma.menuobject = 'employee') 
			and x.companyid=".(isset($_REQUEST['companyid'])?$_REQUEST['companyid']:'null').")  
        and c.fullname like '%".(isset($_REQUEST['fullname'])?$_REQUEST['fullname']:'')."%' 
        group by a.employeeid
        order by b.orgstructureid";
				/*b.parentid in (select x.orgstructureid
			from orgstructure x
			join employeeorgstruc y on y.orgstructureid = x.orgstructureid
			join employee z on z.employeeid = y.employeeid
			where z.employeeid in (select gm.menuvalueid from groupaccess c
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'employeeid', 'fullname','structurename'
        ),
				'defaultOrder' => array( 
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->IDField?>_search_fullname"><?php echo $this->getCatalog('fullname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
