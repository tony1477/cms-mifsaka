<?php
	$sqldata="select familyrelationid,familyrelationname 
		from familyrelation 
		where recordstatus = 1 and familyrelationname like '%".(isset($_REQUEST['familyrelationname'])?$_REQUEST['familyrelationname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$familyrelation=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'familyrelationid', 'familyrelationname'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'familyrelationname':$("input[name='<?php echo $this->ColField; ?>_search_familyrelationname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_familyrelationname"><?php echo $this->getCatalog('familyrelationname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_familyrelationname" class="form-control">
					<span class="input-group-btn">
