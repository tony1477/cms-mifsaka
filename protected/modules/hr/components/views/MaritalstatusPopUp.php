<?php
	$sqldata="select maritalstatusid,maritalstatusname 
		from maritalstatus 
		where recordstatus = 1 and maritalstatusname like '%".(isset($_REQUEST['maritalstatusname'])?$_REQUEST['maritalstatusname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$maritalstatus=new CSqlDataProvider($sqldata,array(
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'maritalstatusname':$("input[name='<?php echo $this->ColField; ?>_search_maritalstatusname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_maritalstatusname"><?php echo $this->getCatalog('maritalstatusname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_maritalstatusname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				'id'=>$this->PopGrid,
				'selectableRows'=>1,
				'dataProvider'=>$maritalstatus,
				'selectionChanged'=>'function(id){'.$this->PopGrid.'onSelectionChange()}',
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('maritalstatusid'),
