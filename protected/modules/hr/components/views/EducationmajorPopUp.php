<?php
	$sqldata="select a.educationmajorid,a.educationmajorname
		from educationmajor a 
		where a.recordstatus = 1 and a.educationmajorname like '%".(isset($_REQUEST['educationmajorname'])?$_REQUEST['educationmajorname']:'')."%' 
			and a.educationid = ".(isset($_REQUEST['educationid'])?$_REQUEST['educationid']:'null');
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$product=new CSqlDataProvider($sqldata,array(
			'totalItemCount'=>$count,
			'keyField'=>'educationmajorid',
			'pagination'=>array(
			...
			'sort'=>array(
        'attributes'=>array(
             'educationmajorid', 'educationmajorname'
        ),
				'defaultOrder' => array( 
					'educationmajorid' => CSort::SORT_DESC
				),
			),
		));
		?>
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'educationmajorname':$("input[name='<?php echo $this->IDField?>_search_educationmajorname']").val(),
		'educationid':$("input[name='<?php echo $this->RelationID?>']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->IDField?>_search_educationmajorname"><?php echo $this->getCatalog('educationmajorname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->IDField?>_educationmajorname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('educationmajorid'),
						'name'=>'educationmajorid', 
						'value'=>'$data["educationmajorid"]',
						'htmlOptions'=>array('width'=>'1%')
					),
					array(
						'header'=>$this->getCatalog('educationmajorname'),
						'name'=>'educationmajorname', 
						'value'=>'$data["educationmajorname"]',
					),
				),
			));?>
			</div>
