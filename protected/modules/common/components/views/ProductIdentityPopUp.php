<?php
	$sqldata="select productidentityid,identityname,recordstatus
		from productidentity
		where recordstatus = 1 
		and identityname like '%".(isset($_REQUEST['identityname'])?$_REQUEST['identityname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$materialtype=new CSqlDataProvider($sqldata,array(
			'totalItemCount'=>$count,
			'keyField'=>'productidentityid',
			'pagination'=>array(
			...
			'sort'=>array(
        'attributes'=>array(
             'productidentityid', 'indentityname','recordstatus'
        ),
				'defaultOrder' => array( 
					'productidentityid' => CSort::SORT_DESC
				),
			),
		));
		?>
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'identityname':$("input[name='<?php echo $this->ColField; ?>_search_identityname']").val(),
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_identityname"><?php echo $this->getCatalog('identityname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField; ?>_search_identityname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('productidentityid'),
						'name'=>'productidentityid', 
						'value'=>'$data["productidentityid"]',
