<?php
	$sqldata="select a.marketareaid,a.marketname
		from marketarea a 
		where a.recordstatus = 1 and a.marketname like '%".(isset($_REQUEST['marketname'])?$_REQUEST['marketname']:'')."%'";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$product=new CSqlDataProvider($sqldata,array(
			'totalItemCount'=>$count,
			'keyField'=>'marketareaid',
			'pagination'=>array(
			...
			'sort'=>array(
        'attributes'=>array(
             'marketareaid', 'marketname'
        ),
				'defaultOrder' => array( 
					'marketareaid' => CSort::SORT_DESC
				),
			),
		));
		?>
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'marketname':$("input[name='<?php echo $this->ColField?>_search_marketname']").val()
	}});
	return false;
}
...
<div class="row">
	...
	<div class="<?php echo $this->classtypebox ?>">
		...
		<div class="input-group">
			<input type="text" name="<?php echo $this->ColField ?>" class="form-control" readonly>
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField?>_search_marketname"><?php echo $this->getCatalog('marketname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField?>_search_marketname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('marketareaid'),
						'name'=>'marketareaid', 
						'value'=>'$data["marketareaid"]',
						'htmlOptions'=>array('width'=>'1%')
					),
					array(
						'header'=>$this->getCatalog('marketname'),
						'name'=>'marketname', 
