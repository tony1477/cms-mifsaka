<?php
	$sqldata="select groupkastaid, groupname 
  from groupkasta 
  where groupname like '%".(isset($_REQUEST['groupname'])?$_REQUEST['groupname']:'')."%' ";
	$count = count(Yii::app()->db->createCommand($sqldata)->queryAll());
	$product=new CSqlDataProvider($sqldata,array(
			'totalItemCount'=>$count,
			'keyField'=>'groupkastaid',
			'pagination'=>array(
			...
			'sort'=>array(
        'attributes'=>array(
             'groupkastaid', 'groupname'
        ),
				'defaultOrder' => array( 
					'groupkastaid' => CSort::SORT_DESC
				),
			),
		));
		?>
	 ...
	 <div class="modal-content">
      ...
      <div class="modal-body">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('groupkastaid'),
						'name'=>'groupkastaid', 
						'value'=>'$data["groupkastaid"]',
