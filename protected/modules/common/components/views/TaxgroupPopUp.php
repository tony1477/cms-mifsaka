<?php
	$sqldata="select a.taxgroupid,a.taxgroupname
		from taxgroup a 
		where a.recordstatus = 1 and a.taxgroupname like '%".(isset($_REQUEST['taxgroupname'])?$_REQUEST['taxgroupname']:'')."%'";
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'taxgroupid', 'taxgroupname'
        ),
				'defaultOrder' => array( 
					'taxgroupid' => CSort::SORT_DESC
				),
			),
		));
		?>
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'taxgroupname':$("input[name='<?php echo $this->ColField?>_search_taxgroupname']").val()
	}});
	return false;
}
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField?>_taxgroupname"><?php echo $this->getCatalog('taxgroupname') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField?>_taxgroupname" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('taxgroupid'),
						'name'=>'taxgroupid', 
