<?php
	$sqldata="select a.plantid,a.plantcode,description
		from plant a 
		where a.recordstatus = 1 
        and a.companyid = ".(isset($_REQUEST['companyid'])?$_REQUEST['companyid']:'null')." 
        and a.plantcode like '%".(isset($_REQUEST['plantcode'])?$_REQUEST['plantcode']:'')."%' 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		'plantcode':$("input[name='<?php echo $this->ColField?>_search_plantcode']").val(),
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField?>_plantcode"><?php echo $this->getCatalog('plantcode') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" name="<?php echo $this->ColField?>_plantcode" class="form-control">
					<span class="input-group-btn">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('plantid'),
						'name'=>'plantid', 
						'value'=>'$data["plantid"]',
