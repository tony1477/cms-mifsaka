<?php
	$sqldata=" select a.slocid,a.productid,c.sloccode, b.productname, d.description
        from productplant a
        ...
        where a.recordstatus = 1 and c.slocid in (".getUserObjectWfValues('sloc').")
					and a.materialgroupid in (select materialgroupid 
            from materialgroup x 
            where parentmatgroupid = ".(isset($_REQUEST['parentmatgroupid'])?$_REQUEST['parentmatgroupid']:'null')."
            and recordstatus=1 and isfg=0) 
		and productname like '%".(isset($_REQUEST['productname'])?$_REQUEST['productname']:'')."%' and e.companyid = ".(isset($_REQUEST['companyid'])?$_REQUEST['companyid']:'null')."";
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'productid', 'productname','slocid','description'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
    ...
    'parentmatgroupid':$("input[name='<?php echo $this->RelationID?>']").val(),
