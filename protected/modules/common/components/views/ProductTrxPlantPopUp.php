<?php
    if(isset($_REQUEST['materialtypeid'])  && isset($_REQUEST['soheader'])) {
        $materialtypeid = ' and t.materialtypeid  = '.$_REQUEST['materialtypeid'];
	...
	$sqldata="select t.productid,case when t.productpic = '' then 'default.jpg' else t.productpic end as productpic,t.productname 
		from product t
		left join productplant a1 on a1.productid = t.productid
		left join materialtype a on a.materialtypeid = t.materialtypeid
		left join productidentity b on b.productidentityid = t.productidentityid
		left join productbrand c on c.productbrandid = t.productbrandid
		left join productcollection d on d.productcollectid = t.productcollectid
		left join productseries e on e.productseriesid = t.productseriesid
		where t.recordstatus = 1 and a1.recordstatus = 1 and slocid in (".getUserObjectValues('sloc').")
		{$materialtypeid}
		and (t.productid like '%".(isset($_REQUEST['productname'])?$_REQUEST['productname']:'')."%' or t.productname like '%".(isset($_REQUEST['productname'])?$_REQUEST['productname']:'')."%' or t.barcode like '%".(isset($_REQUEST['productname'])?$_REQUEST['productname']:'')."%')";
