...
$icon->setOrigin(0, 0);
$companys = Yii::app()->db->createCommand('select fullname,b.lat,b.lng,creditlimit,currentlimit,b.addressname from addressbook a 
	inner join address b on b.addressbookid = a.addressbookid where isvendor=1')->queryAll();
foreach($companys as $company)
{
	$marker = new EGMapMarkerWithLabel($company['lat'],$company['lng'], 
		array('title' => $company['fullname'],
		'icon'=>$icon));
	$info_window = new EGMapInfoWindow('Supplier : '.$company['fullname'].
