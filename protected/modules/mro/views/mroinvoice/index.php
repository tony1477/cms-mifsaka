...
function savedata()
{
    ...
    function newdatamroinvdetail(){
	jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('mro/mrogi/createmroinvdetail')?>','data':{},
		'type':'post','dataType':'json',
		'success':function(data) {
			if (data.status == "success")
			{
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <?php $this->widget('DataPopUp',
				...
				<li><a data-toggle="tab" href="#mroinvdetail" onclick="check_mroinvdetail()"><?php echo $this->getCatalog("mroinvdetail")?></a></li>

				</ul>
				<div class="tab-content">
                  
                    <div id="mroinvdetail" class="tab-pane">
	<!--
    <?php if ($this->checkAccess('mroinv','iswrite')) { ?>
<button name="CreateButtonmroinvdetail" type="button" class="btn btn-primary" onclick="newdatamroinvdetail()"><?php echo $this->getCatalog('new')?></button>
<?php } ?>
<?php if ($this->checkAccess('mroinv','ispurge')) { ?>
<button name="PurgeButtonmroinvdetail" type="button" class="btn btn-danger" onclick="purgedatamroinvdetail()"><?php echo $this->getCatalog('purge')?></button>
<?php } ?>
    -->
      <?php
	$this->widget('zii.widgets.grid.CGridView', array(
    'dataProvider'=>$dataProvidermroinvdetail,
		'id'=>'mroinvdetailList',
		'selectableRows'=>2,
    'ajaxUpdate'=>true,
		'filter'=>null,
			...
			(
				...
				(
					...
					(
							...
							'url'=>'"#"',
							'click'=>"function() { 
								updatedatamroinvdetail($(this).parent().parent().children(':nth-child(3)').text());
							}",
					),
					'purge' => array
					(
							...
							'url'=>'"#"',
							'click'=>"function() { 
								purgedatamroinvdetail($(this).parent().parent().children(':nth-child(3)').text());
							}",
					),
				),
			),
...
<div id="ShowDetailDialog" class="modal fade" role="dialog">
	...
	<div class="box-header with-border">
		<h3 class="box-title"><?php echo $this->getCatalog('mroinvdetail')?></h3>
...
<div id="InputDialogmrovidetail" class="modal fade" role="dialog">
	<div class="modal-dialog">
