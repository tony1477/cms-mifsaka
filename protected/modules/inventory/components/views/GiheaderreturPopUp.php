<?php
	$sqldata="select b.gino, b.giheaderid
from giheader b  
...
and b.gino like '%".(isset($_REQUEST['gino'])?$_REQUEST['gino']:'')."%' 
 " 
;
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'giheaderid', 'gino','gidate','headernote'
        ),
				'defaultOrder' => array( 
...
function <?php echo $this->IDField?>searchdata()
{
	$.fn.yiiGridView.update("<?php echo $this->PopGrid?>",{data:{
		...
		'gino':$("input[name='<?php echo $this->ColField; ?>_search_pono']").val()
		
	}});
	return false;
}
function update_gireturdetail1(){
    jQuery.ajax({'url':'<?php echo Yii::app()->createUrl('inventory/giretur/updategireturdetail1')?>',
		'data':{
            'gireturid':$("input[name='giretur_0_gireturid']").val(),
			'gidate':$("input[name='giretur_0_gidate']").val(),
	 ...
	 <div class="modal-content">
      ...
      <div class="modal-body">
			...
			$this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						'header'=>$this->getCatalog('giheader'),
						'name'=>'giheaderid', 
