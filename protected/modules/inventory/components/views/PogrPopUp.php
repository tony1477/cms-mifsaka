<?php
	$sqldata="select b.pono, c.fullname,b.poheaderid
from poheader b 
join addressbook c on c.addressbookid = b.addressbookid 
where b.poheaderid in 
(
select a.poheaderid 
from podetail a
where a.poqty > a.qtyres 
)
and b.recordstatus = getwfmaxstatbywfname('apppo') 
and b.recordstatus in (".getUserRecordStatus('listpo').")
				and
				b.companyid in (".getUserObjectValues('company').")
 and b.pono like '%".(isset($_REQUEST['pono'])?$_REQUEST['pono']:'')."%' 
...
;
	...
	$product=new CSqlDataProvider($sqldata,array(
			...
			'sort'=>array(
        'attributes'=>array(
             'poheaderid', 'pono', 'addressbookid'
        ),
				'defaultOrder' => array( 
					'a.poheaderid' => CSort::SORT_DESC
				),
			),
		));
		?>
	...
	<div class="modal-dialog modal-lg">	
	 <div class="modal-content">
      ...
      <div class="modal-body">
			...
			<div class="row">
			<div class="col-md-4">
				<label class="control-label" for="<?php echo $this->ColField; ?>_search_fullname"><?php echo $this->getCatalog('supplier') ?></label>
			</div>
			<div class="col-md-8">
				<div class="input-group">
