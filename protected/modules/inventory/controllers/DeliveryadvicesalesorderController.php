...
class DeliveryadvicesalesorderController extends AdminController
{
	protected $menuname = 'deliveryadvicesalesorder';
	public $module = 'Inventory';
	protected $pageTitle = 'Form Permintaan Barang SO';
	public $wfname = 'appda';
	protected $sqldata = "select a0.deliveryadviceid,a0.dadate,a0.dano,a0.useraccessid,a0.slocid,a0.productplanid,a0.soheaderid,a0.productoutputid,a0.headernote,a0.recordstatus,a1.username as username,a2.sloccode as sloccode,a3.sono as sono,a0.statusname  
    from deliveryadvice a0 
    left join useraccess a1 on a1.useraccessid = a0.useraccessid
    left join sloc a2 on a2.slocid = a0.slocid
    left join soheader a3 on a3.soheaderid = a0.soheaderid
  ";
  ...
  ";
	...
	protected function getSQL()
	{
		...
		$where = "where a0.recordstatus in (".getUserRecordStatus('listda').")
				...
				and a3.sono is not null";
		if ((isset($_REQUEST['dano'])) && (isset($_REQUEST['username'])) && (isset($_REQUEST['sloccode'])) && (isset($_REQUEST['sono'])))
		{				
			$where .=  " 
and a0.dano like '%". $_REQUEST['dano']."%' 
and a1.username like '%". $_REQUEST['username']."%' 
and a2.sloccode like '%". $_REQUEST['sloccode']."%' 
and a3.sono like '%". $_REQUEST['sono']."%'"; 
		}
...
public function actionSave()
	{
		...
		if ($error == false)
		{
			...
			try
			{
				$sql = 'call UpdateDASO (:deliveryadviceid
,:dadate
,:headernote
,:slocid
,:soheaderid
...
public function actionDelete()
	...
	public function actionDownPDF()
	{
		...
		$sql = "select getcompanysloc(a.slocid) as companyid,a.dano,a.dadate,a.headernote,
				...
				$this->pdf->title=$this->getcatalog('deliveryadvicesalesorder');
			...
			foreach($dataReader as $row)
			{
					...
					$this->pdf->text(120,$this->pdf->gety()+2,'SO ');$this->pdf->text(140,$this->pdf->gety()+2,': '.$row['sono']);
...
->setCellValueByColumnAndRow(6,4,$this->getCatalog('sono'))
...
->setCellValueByColumnAndRow(6, $i+1, $row1['sono'])
