...
class DeliveryadviceController extends AdminController
{
	protected $menuname = 'deliveryadvice';
	public $module = 'Inventory';
	protected $pageTitle = 'Form Permintaan Barang';
	public $wfname = 'appda';
...
protected $sqlcountdeliveryadvicedetail = "select count(1) 
  ...
  ";
	...
	protected function getSQL()
	{
		...
		$where = " where a0.productplanid is null 
			and a0.soheaderid is null 
			and	a0.productoutputid is null 
	and a0.recordstatus < {$maxstat}
			and a0.recordstatus in (".getUserRecordStatus('listda').") 
  ...
  public function actionUpdatedeliveryadvicedetail()
	{
		...
		if (isset($_POST['id']))
		{
			...
			if ($model !== null)
			{
				echo CJSON::encode(array(
					'status'=>'success',
					'deliveryadvicedetailid'=>$model['deliveryadvicedetailid'],
...
public function actionSave()
	{
      ...
      array('slocid','string','emptyslocid'),
		...
		if ($error == false)
		{
			...
			try
			{
				$sql = 'call UpdateDA (:deliveryadviceid,:dadate
,:headernote
,:slocid
,:vcreatedby)';
	...
	public function actionPurge()
	{
		...
		try
		{
			if (isset($_POST['id']))
			{
				...
				{
				$sql = "delete from deliveryadvicedetail where deliveryadviceid = ".$id[$i];
				Yii::app()->db->createCommand($sql)->execute();
