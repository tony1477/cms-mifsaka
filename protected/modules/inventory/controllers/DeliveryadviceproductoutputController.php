...
class DeliveryadviceproductoutputController extends AdminController
{
	protected $menuname = 'deliveryadviceproductoutput';
	public $module = 'Inventory';
	protected $pageTitle = 'Form Permintaan Barang OP';
	public $wfname = 'appda';
	protected $sqldata = "select a0.deliveryadviceid,a0.dadate,a0.dano,a0.useraccessid,a0.slocid,a0.productplanid,a0.soheaderid,a0.productoutputid,a0.headernote,a0.recordstatus,a1.username as username,a2.sloccode as sloccode,a3.productoutputno as productoutputno,a0.statusname  
    from deliveryadvice a0 
    left join useraccess a1 on a1.useraccessid = a0.useraccessid
    left join sloc a2 on a2.slocid = a0.slocid
    left join productoutput a3 on a3.productoutputid = a0.productoutputid
  ";
...
protected $sqlcountdeliveryadvicedetail = "select count(1) 
  ...
  ";
	...
	and a0.recordstatus < {$maxstat}
				...
				and a0.productoutputid is not null";
		if ((isset($_REQUEST['dano'])) && (isset($_REQUEST['username'])) && (isset($_REQUEST['sloccode'])) && (isset($_REQUEST['productoutputno'])))
		{				
			$where .=  " 
...
and a3.productoutputno like '%". $_REQUEST['productoutputno']."%'"; 
    ...
    $dataProvider=new CSqlDataProvider($this->sqldata,array(
			...
			'sort'=>array(
				'attributes'=>array(
					 'deliveryadviceid','dadate','dano','useraccessid','slocid','productplanid','soheaderid','productoutputid','headernote','statusname'
				),
				'defaultOrder' => array( 
  ...
  public function actionUpdatedeliveryadvicedetail()
	{
		...
		if (isset($_POST['id']))
		{
			...
			{
				echo CJSON::encode(array(
					'status'=>'success',         'deliveryadvicedetailid'=>$model['deliveryadvicedetailid'],
...
public function actionSave()
	{
      ...
      array('slocid','string','emptyslocid'),
		...
		if ($error == false)
		{
			...
			try
			{
				$sql = 'call UpdateDAOP (:deliveryadviceid,:dadate
,:headernote
,:slocid
,:productoutputid
,:vcreatedby)';
		...
		if ($error == false)
		{
			...
			try
			{
				if ($id !== '')
				{
			      ...
			      set deliveryadviceid = :deliveryadviceid,productid = :productid,qty = :qty,unitofmeasureid = :unitofmeasureid,requestedbyid = :requestedbyid,reqdate = :reqdate,itemtext = :itemtext,slocid = :slocid 
			      where deliveryadvicedetailid = :deliveryadvicedetailid';
				}
				else
				{
					$sql = 'insert into deliveryadvicedetail (deliveryadviceid,productid,qty,unitofmeasureid,requestedbyid,reqdate,itemtext,slocid) 
			      values (:deliveryadviceid,:productid,:qty,:unitofmeasureid,:requestedbyid,:reqdate,:itemtext,:slocid)';
				}
				$command = $connection->createCommand($sql);
				if ($id !== '')
				{
...
public function actionDelete()
    ...
    $this->pdf->companyid = $row['companyid'];
	  ...
	  $this->pdf->title=$this->getcatalog('deliveryadviceproductoutput');
...
->setCellValueByColumnAndRow(7,4,$this->getCatalog('productoutputno'))
...
->setCellValueByColumnAndRow(7, $i+1, $row1['productoutputno'])
